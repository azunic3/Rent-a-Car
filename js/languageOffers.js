...
const more = document.querySelector('.more');
    ...
    window.onload=(function(){
      ...
      clio.textContent = data[attr].clio;
      tesla.textContent = data[attr].tesla;
      volks.textContent = data[attr].volks;
      benz.textContent = data[attr].benz;
      polo.textContent = data[attr].polo;
	  ...
	  perweekend4.textContent = data[attr].perweekend;
	  perweekend5.textContent = data[attr].perweekend;
	  perweekend6.textContent = data[attr].perweekend;
	  from1.textContent = data[attr].from;
      from2.textContent = data[attr].from;
	  from3.textContent = data[attr].from;
	  from4.textContent = data[attr].from;
	  from5.textContent = data[attr].from;
	  from6.textContent = data[attr].from;

	  offers.textContent = data[attr].offersclass2;
...
"benz":"Indulge in the luxury and performance of a Mercedes-Benz for your weekend escape. These prestigious vehicles offer top-tier comfort and style, setting the stage for an unforgettable trip.",
	...
	"german": {
		...
		"contact2": "Wir sind hier, um Sie bei Fragen, Anfragen oder besonderen Wünschen zu unterstützen. Bei An-An Rent a Car hat Ihre Zufriedenheit höchste Priorität, und wir sind bereit, Ihnen außergewöhnlichen Kundenservice zu bieten. Zögern Sie nicht, uns telefonisch, per E-Mail oder durch einen Besuch an unserem praktischen Standort zu kontaktieren. Ihre Reise beginnt hier, und wir sind bestrebt, ein Teil davon zu sein. Lassen Sie uns in Verbindung bleiben und Ihr Mietwagenerlebnis reibungslos und angenehm gestalten.",
		...
		"more": "Mehr",
		"clio": "Entdecken Sie den Renault Clio, eine beliebte und zuverlässige Wahl für Ihre Wochenendtrips. Dieser kompakte Wagen punktet mit exzellenter Kraftstoffeffizienz und einem komfortablen Interieur, was eine reibungslose und angenehme Fahrt gewährleistet.",
		...
		"polo": "Erleben Sie den Volkswagen Polo, ein vielseitiger und kompakter Wagen, der perfekt für Ihre Wochenendausflüge geeignet ist. Mit seiner effizienten Leistung und dem komfortablen Interieur bietet der Polo ein angenehmes Fahrerlebnis.",
		...
		"benz": "Tauchen Sie ein in den Luxus und die Leistung eines Mercedes-Benz für Ihren Wochenendausflug. Diese edlen Fahrzeuge bieten erstklassigen Komfort und Stil und schaffen die Grundlage für eine unvergessliche Reise."
	}
