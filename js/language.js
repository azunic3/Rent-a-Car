...
const citro = document.getElementById('citro');
...
const years=document.getElementById('5years');
const kljuc=document.getElementById('kljuc');
...
window.onload=(function(){
	if (localStorage.getItem('language') === null) {
      ...
      bannerText1.textContent = data[attr].bannerText1;
	  bannerText2.textContent = data[attr].bannerText2;
toyota.textContent=data[attr].toyota;
citro.textContent=data[attr].citro;
ford.textContent=data[attr].ford;
aboutus3.textContent=data[attr].an_an_aboutus;
	  offersclass1.textContent = data[attr].offersclass1;
	  offersclass2.textContent = data[attr].offersclass1;
	  banner.textContent=data[attr].banner2;
	  ...
	  years.textContent=data[attr].years;
	  kljuc.textContent=data[attr].kljuc;
	...
	//  fuelPolicyWritten.textContent=data[attr].fuelPolicyWritten;
	  ...
	  merc.textContent=data[attr].merc;
  ...
  link[0].addEventListener('click', () => {
      ...
      link[0].classList.add('active-language');
...
aboutus3.textContent=data[attr].an_an_aboutus;
    ...
    link[1].addEventListener('click', () => {
		langEl.querySelector('.active-language').classList.remove('active-language');
		link[1].classList.add('active-language');
  
	  localStorage.setItem('language','bosnian');
		const attr= localStorage.getItem('language');
  ...
  aboutus3.textContent=data[attr].an_an_aboutus;
	  ...
	  link[2].addEventListener('click', () => {
		langEl.querySelector('.active-language').classList.remove('active-language');
		link[2].classList.add('active-language');
...
"contact2":"We're here to assist you with any questions, inquiries, or special requests you may have. At An-An Rent a Car, your satisfaction is our priority, and we're ready to provide you with exceptional customer service. Feel free to reach out to us via phone, email, or by visiting our convenient location. Your journey begins here, and we're eager to be a part of it. Let's connect and make your rental experience seamless and enjoyable.",
	...
	"german":{
		"bannerText1": "Beschleunigen Sie Ihre Ersparnisse",
		"banner2":"Erhalten Sie 20% Rabatt auf Van-Mieten heute!",
		"bannerText2":"Finden Sie uns in Ihrem nächsten Geschäft",
		"aboutus":"Über uns",
		"review2":"\"An-An Rent a Car ist meine erste Wahl in Bosnien und Herzegowina. Die Autos sind gut gepflegt und das Personal ist immer bereit zu helfen. Ich habe einen Mercedes-Benz gemietet und es hat meine Reise unvergesslich gemacht.\"",
		"review3":"\"Der Mercedes-Benz, den ich bei An-An Rent a Car gemietet habe, war ein Blickfang. Er hat überall, wo ich hingekommen bin, die Blicke auf sich gezogen, und der Service war erstklassig. An-An ist meine bevorzugte Autovermietung ab jetzt.\"",
		"review4":"\"Ich hatte eine fantastische Erfahrung beim Mieten eines Fahrzeugs von An-An Rent a Car für meinen Familienurlaub. Der Schlüssel zu unserer wundervollen Reise war ein komfortables und zuverlässiges Auto von An-An. Wir sind äußerst zufrieden mit ihrem Service!\"",
		"review5":"\"Ich habe kürzlich einen Ford von An-An Rent a Car gemietet, und er hat meine Erwartungen übertroffen. Das Auto war sauber, zuverlässig, und das Team von An-An hat den Mietprozess einfach und bequem gemacht.\"",
		...
		"teslareview":"\"Ich habe einen Tesla bei An-An Rent a Car gemietet und hatte ein erstaunliches Erlebnis. Das Auto war in perfektem Zustand, und das Personal war äußerst freundlich und professionell. Ich komme auf jeden Fall für mehr zurück!\"",
		"years":"5 Jahre Exzellenz: Feiern eines Meilensteins!",
		"merc":"Erleben Sie Luxus und Eleganz wie nie zuvor mit unserem brandneuen Mercedes-Benz-Modell, das jetzt für Ihre Mietwünsche verfügbar ist.",
		...
		"ford":"Ob Sie die Stadt erkunden oder die offene Straße befahren, unsere Ford-Mietwagen bieten die perfekte Kombination aus Leistung und Komfort.",
		"citro":"Der Citroen ist eine vielseitige Wahl für Ihre Mietwünsche und bietet eine harmonische Kombination aus Sicherheit und modernem Design. Egal, ob Sie urbane Landschaften erkunden oder sich abseits ausgetretener Pfade wagen, der Citroen bietet ein fantastisches Fahrerlebnis. Buchen Sie jetzt Ihren Citroen und begeben Sie sich auf Ihr nächstes Abenteuer!",
		"toyota":"Mieten Sie einen luxuriösen SUV mit erstklassiger Leistung und Komfort - den Toyota Fortuner. Dieses geräumige Fahrzeug vereint Eleganz mit Offroad-Fähigkeiten und ist die perfekte Wahl, um Städte und Naturwunder zu erkunden.",
		"offersclass1": "Angebote",
